...
static int getInt(int* pResultado)
{
    ...
    if(getString(buffer,sizeof(buffer))==0 && esNumerica(buffer,4056))
    {
...
int utn_getNumeroTxt(char* pResultado, char* mensaje, char* mensajeError, int reintentos)
{
	int retorno = -1;
	char bufferInt[4096];
	...
	if(pResultado != NULL && mensaje != NULL && mensajeError != NULL  && reintentos >= 0)
	{
		...
		{
			printf("%s",mensaje);
			if(getString(bufferInt,4096)==0 && esNumerica(bufferInt,4096))
			{
				strncpy(pResultado,bufferInt,sizeof(bufferInt));
				retorno = 0;
				break;
			}
			else
...
int utn_getFloatTxt(char* pResultado, char* mensaje, char* mensajeError, int reintentos)
{
	int retorno = -1;
	char bufferInt[4096];
	...
	if(pResultado != NULL && mensaje != NULL && mensajeError != NULL  && reintentos >= 0)
	{
		...
		{
			printf("%s",mensaje);
			if(getString(bufferInt,4096)==0 && esFloat(bufferInt))
			{
				strncpy(pResultado,bufferInt,sizeof(bufferInt));
				retorno = 0;
				break;
